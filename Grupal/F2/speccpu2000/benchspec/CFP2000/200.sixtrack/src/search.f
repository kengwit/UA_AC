...
C-----------------------------------------------------------------------
C  FINDING THE BEST POSITIONS FOR CORRECTION-ELEMENTS
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      write(6,10020) mp
      write(6,10010)
      write(6,10030) m21,ise1,m22,ise2,m23,ise3
      write(6,10010)
      write(6,10040)
      write(6,10010)
      n21=m21+mp
      ...
      c3=rtc(mp,n23,mp,1)
      s3=rts(mp,n23,mp,1)
      write(6,10050) ref,id,c1,s1,c2,s2,c3,s3
      do 10 i=1,mesa
        ...
        write(6,10050) bez(isea(i)),i,b,c,d,e,f,g
        ed(isea(i))=ed(isea(i))-dsm0
        if(kp(isea(i)).eq.5) call combel(isea(i))
   10 continue
      nta=ntao
...
10030 format(24x ,'|',6x,'NZ =',i4,';D-P= ',i4,7x, '|',6x,'NZ =',i4,
     +';D-P= ',i4,7x,'|',6x,'NZ =',i4,';D-P= ',i4,7x, '|')

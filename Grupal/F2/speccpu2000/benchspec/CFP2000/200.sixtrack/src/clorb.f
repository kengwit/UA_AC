...
C-----------------------------------------------------------------------
C  CALCULATION OF THE CLOSED ORBIT   'CLO(2),CLOP(2)'
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
   ...
   40 continue
      if(ncorru.ne.1)
     + write(6,10000) itco
   50 cor=sqrt(dcx*dcx+dcz*dcz)
      if(iout.eq.1.and.ncorru.ne.1) then
        write(6,10010) dpp,clo(1),clop(1),clo(2),clop(2),ii,cor
      endif
C-----------------------------------------------------------------------
      return
...
10010 format(t5,'---- ENTRY CLORB ----/DPP=',f8.5,' /CLOX/', 2f10.5,
     +' /CLOZ/',2f10.5,' /ITERAT.=',i3,'/ ACCURACY=',d13.6)
      end

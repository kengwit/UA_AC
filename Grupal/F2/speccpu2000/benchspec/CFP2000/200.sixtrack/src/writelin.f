   ...
   10   continue
      ...
      if(iwrite.eq.1) then
        do 20 l=1,2
          ...
          cp(l)=t(1,ll)
   20   continue
        if(kz(ixwl).eq.20.and.nbeam.ge.1) then
          if(nvbeam.eq.-1) then
            sigman6(1,ixwl)=sqrt(emitnx*gammar*b1(1))
            sigman6(2,ixwl)=sqrt(emitnz*gammar*b1(2))
            if(sigman6(1,ixwl).le.pieni.or.
     +      sigman6(2,ixwl).le.pieni) call error(88)
          else if(nvbeam.ne.0) then
            sigman(1,nvbeam,ixwl)=sqrt(emitnx*gammar*b1(1))
            sigman(2,nvbeam,ixwl)=sqrt(emitnz*gammar*b1(2))
            if(sigman(1,nvbeam,ixwl).le.pieni.or.
     +      sigman(2,nvbeam,ixwl).le.pieni) call error(88)
            if(iclo6.ne.1.and.iclo6.ne.2) then
              clobeam(1,nvbeam,ixwl)=c(1)
              clobeam(2,nvbeam,ixwl)=c(2)
            endif
          else if(nvbeam.eq.0.and.(iclo6.ne.1.and.iclo6.ne.2)) then
            sigman6(1,ixwl)=sqrt(emitnx*gammar*b1(1))
            sigman6(2,ixwl)=sqrt(emitnz*gammar*b1(2))
            if(sigman6(1,ixwl).le.pieni.or.
     +      sigman6(2,ixwl).le.pieni) call error(88)
        ...
        if(ncorru.eq.0) then
          write(6,10000) nr,typ(:8),tl,p1(1),b1(1),al1(1),g1(1),d(1),
     +    dp(1),c(1),cp(1)
          write(6,10010) b2(1),al2(1),g2(1)
          write(6,10030) typ(9:16)
          write(6,10020) p1(2),b1(2),al1(2),g1(2),d(2),dp(2),
     +    c(2),cp(2)
          write(6,10010) b2(2),al2(2),g2(2)
          write(6,10040)
        else

...
static U32 seedi = 0;
static SPEC_INT32_T *randnums = 0;
static SPEC_INT32_T numnums = 0;
static SPEC_INT32_T numidx = 0;
static SPEC_INT32_T stride = 517;
#endif /* SPEC_CPU2000 */
...
#else /* SPEC_CPU2000 */
    if (!srand_called) {
        die("Hey!  Call srand first!");
    }
    if (value > 1.0) {
        /* warn("Making number 0-%u\nindex #%u of %u\n", (unsigned)value,
           numidx, numnums); */
      value = (double)(randnums[numidx] % (unsigned int)value);
    } else { /* and this is a *real* hack */
        /* The range of the number is (for a 32-bit machine) 2^32-1 */
        /* Since the numbers should have been generated on a 32-bit machine
           (lowest common denominator, don'cha know), we can just divide
           by the maximum unsigned value to get a number in the range
           0-1.  Because we know the maximum, there's no need to do a
           modulus or something like that.
         */
      /* value = (double)(randnums[numidx] % 4000000) / 4000000; */
        /* warn("Making number 0-1\nindex #%u of %u\n", numidx, numnums); */
      value = ((double)randnums[numidx]) / 4294967295.0;
    }
    numidx += stride;
    numidx = numidx % numnums;
    if (numidx >= numnums) /* Start over! */
      numidx %= numnums;
    XPUSHn(value);
    RETURN;
#endif /* SPEC_CPU2000 */
...
PP(pp_srand)
{
    ...
    int nums = 0;
    int intoffset = 0;
    unsigned char *tmpnums = 0;
    unsigned SPEC_INT32_T *curnum = 0;

    if (MAXARG < 1)
        anum = 1; /* Bad seed!  Bad, bad seed! */
    else
        anum = POPu;
    seedi = (long)anum;
...
#ifdef SPEC_CPU2000
    if (randnums == 0) {
      fd = open(SPEC_CPU2000_NUMFILE, SPEC_CPU2000_NUMFILE_FLAGS);
      /* warn("Opened %s as fd %d\n", SPEC_CPU2000_NUMFILE, fd); */
      if (fd < 0) {
        die("Failed to open %s for reading\n", SPEC_CPU2000_NUMFILE);
      } else {
        if (read(fd, (void *)&numsize, 1) != 1) {
          DIE("Argh!  Couldn't read numsize from fd");
        }
      /* warn("read numsize as %d\n", numsize);*/
        /* Now that we have SPEC_INT32_T, intoffset should always be zero */
        intoffset = sizeof(SPEC_INT32_T) - numsize; 
        if (read(fd, (void *)((&numnums) + intoffset), numsize) != numsize) {
          DIE("Argh!  Couldn't read numnums from fd");
        }
        nums = numnums;
      /* warn("read numnums as %d\n", numnums);*/
        /* This is the fastest and least memory efficient way to read the
           numbers in.  The aim is to reduce I/O time, though, right? */
        tmpnums = (unsigned char *)malloc(numnums * numsize);
        randnums = (SPEC_INT32_T *)malloc(numnums * sizeof(SPEC_INT32_T));
        curnum = (unsigned SPEC_INT32_T *)tmpnums;
        if (read(fd, tmpnums, numnums * numsize) != numnums * numsize) {
          DIE("Argh!  Didn't read enough data for nums");
        }
      /*warn("read tmpnums into %P\n", tmpnums);*/
        while(nums > 0) {
            --nums;
          randnums[nums] = *curnum;
          curnum++;
        }
        free(tmpnums);
        close(fd);
    ...
    if (seedi)
      numidx = seedi % numnums;
    stride = seedi % 723; /* arbitrary value */
    if ((stride % 2) == 0) {
      stride++;
    }
#endif /* SPEC_CPU2000 */

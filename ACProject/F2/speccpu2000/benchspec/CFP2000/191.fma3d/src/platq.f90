...
!!
        DO i = 1,8
          EMOTION(i) = MOTION(PLATQ(N)%PAR%IX(i))
        ENDDO
!!
...
!!
        TIMSIM%DTP4x = MAX (TIMSIM%DTP4x,PLATQ(N)%RES%DTelt)
        i = 0
        FOUND = .FALSE.
        DO WHILE (.NOT.FOUND .and. i.LT.NPNDT)
          i = i + 1
          FOUND = PLATQ(N)%RES%DTelt .LT. TIMSIM%DTPl4(i)
        ENDDO
        IF (FOUND) THEN
          IF (i .LT. NPNDT) THEN
            DO j = NPNDT-1,i,-1
              TIMSIM%DTPl4(j + 1) = TIMSIM%DTPl4(j)
              TIMSIM%Plat4(j + 1) = TIMSIM%Plat4(j)
            ENDDO
          ENDIF
          TIMSIM%DTPl4(i) = PLATQ(N)%RES%DTelt
          TIMSIM%Plat4(i) = N
        ENDIF
!!
...
!!
          TIMSIM%DTP4x = MAX (TIMSIM%DTP4x,PLATQ(N)%RES%DTelt)
          IF (PLATQ(N)%RES%DTelt .LT. TIMSIM%DTPl4(1)) THEN
            TIMSIM%DTPl4(1) = PLATQ(N)%RES%DTelt
            TIMSIM%Plat4(1) = N
          ENDIF
!!
...
!!
      DO i = 1,4
        NODE(PLATQ(NEL)%PAR%IX(i))%Mass =                                      &
     &    NODE(PLATQ(NEL)%PAR%IX(i))%Mass + QMass
...
!!
      RMass = QMass * (PLATQ(NEL)%PAR%Area +                                   &
     &  SECTION_2D(SecID)%Thickness**2) / 12.0D+0

      DO i = 5,8
        NODE(PLATQ(NEL)%PAR%IX(i))%Mass =                                      &
     &    NODE(PLATQ(NEL)%PAR%IX(i))%Mass + RMass
      ENDDO
!!
      RETURN
      END
...
!! Local variables.
      REAL(KIND(0D0)) ::                                                       &
     &          Vx(4),Vy(4),Vz(4),Wx(4),Wy(4),Wz(4),X(4),Y(4),Z(4),            &
...
!! functions).
!!
      QA = 1.25000000000000D-1 * (X13*S24 - S13*X24)
      QB = 4.16666666666667D-2 * (X34*S12 - S34*X12)
      QC = 4.16666666666667D-2 * (X23*S14 - S23*X14)
      C0t(1) = QA - (QB + QC)
      C0t(2) = QA + (QB - QC)
      C0t(3) = QA + (QB + QC)
      C0t(4) = QA - (QB - QC)
!!
      QA = 1.25000000000000D-1 * (R13*Y24 - Y13*R24)
      QB = 4.16666666666667D-2 * (R34*Y12 - Y34*R12)
      QC = 4.16666666666667D-2 * (R23*Y14 - Y23*R14)
...
!! bilinear shape functions times Xi1 and Xi2, respectively).
!!
      QD = 4.16666666666667D-02 * (R23*S12 - S23*R12)
      QE = 4.16666666666667D-02 * (R12*S24 - S12*R24)
      QF = 4.16666666666667D-02 * (R13*S34 - S13*R34)
      QG = 4.16666666666667D-02 * (R34*S23 - S34*R23)
      QH = 0.333333333333333D+0 * QC
      QI = 0.333333333333333D+0 * QB
      A1t(1) = ( QD - QH)
      A1t(2) = ( QE + QH)
      A1t(3) = ( QF - QH)
      A1t(4) = ( QG + QH)
...
!!
      ...
      Vin = ONE / Volume
!!
      Qhg = SOUND_SPEED%Density * Dx * HG_Visc * Cv
...
!!
      Qmod = 12.0D+0 * PLATQ(NEL)%RES%DTnext * SOUND_SPEED%RCS2
...
!!
      ...
      ENDDO
      Qe = PLATQ(NEL)%RES%DTnext * HG_Stiff * SOUND_SPEED%RCS2
      DO i = 1,2
